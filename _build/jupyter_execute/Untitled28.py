...
# 
# We are much more likely to be faced with a situation whereby we have a single sample and want to estimate the sampling distriubtion of the mean - that is the distriubtion of sample means for hundereds of other samples that <i>we will never have</i>
# 
# We saw last time that:
# 
# -- if $n$ is large, we can predict the sampling distribution of the mean as $\mathcal{N}(m,\frac{s}{\sqrt{n}})$, 
# 
# -- if $n$ is small but the data distribution is normal, we can predict the sampling sampling distribution of the mean as $t_{n-1}(m,\frac{s}{\sqrt{n}})$
# 
...
# 
# We generate these new samples by sampling <i>with replacement</i> from our set of 20 students' scores.
# 
...
#     <li>draw samples of sie 20 from that simulated population
#         <ul><li>get the mean of each such sample
#     <li>plot a histogram of the sample means
#         </ul>
# </ol>
#         
# The only thing is, it turns out that steps 1 and 2 are really well approximated by just drawing samples of $n=20$ with replacement from the original sample of 20 students         
#         
# Let's try!
...
# Now let's repeat it for a lot of randomm (re)samples
...
# ### The converse
# 
# Just for comparison (and this is cheating because as I said, we wouldn't normally have access to the population distribution), let's try simulating the sampling distribution of the mean bby drawing samples from the UK population distribution.
# 
...
# <i>The term 'bootstrap' was coined by the inventor of the technique, Professor Bradley Efron, in his 1982 mmonograph "The Jackknife, the Bootstrap, and Other Resampling Plans". 
#     
# The termrefers to the apparent impossibility of drawing variable samples from a dataset when the sample sie mmuch match the sie of the dataset (the problem we tackled above) - resampling from the sample is like pulling yourself up by your own bootstraps.
#     
#     
...
#     
# This is a very matter of fact way of pointing out that the simmple approach descroibed here can eb used to obtain the sampling distriubtion of almomst anything, with very simple code, without the need for equations, assumptions or any maths at all. It is virtually magic.
